...
function parseArgs(argv: string[]): Args {
  const out: Args = { command: '', positional: [], json: false, quiet: false, maxToolCalls: 5000 }
  const rest = [...argv]
  ...
  while (rest.length) {
    ...
    switch (token) {
      ...
      case '--markdown': out.markdownFile = rest.shift(); break
      case '--fail-on-drift': out.failOnDrift = Number(rest.shift()); break
      case '--max-tool-calls': out.maxToolCalls = Number(rest.shift()); break
...
function main(): number {
  ...
  if (args.command === 'list') {
    ...
    if (sessions.length > 40) console.log(`  … ${sessions.length - 40} more`)
    return 0
  }

  if (args.command === 'fingerprint') {
    const ref = args.positional[0]
    if (!ref) { console.error('Usage: dsh-driftwatch fingerprint <session>'); return 2 }
  ...
  if (args.command === 'compare') {
    ...
    if (!refA || !refB) { console.error('Usage: dsh-driftwatch compare <sessionA> <sessionB>'); return 2 }
    ...
    if (args.json) {
      ...
      if (args.markdownFile) console.log(`\n_Report also written to ${args.markdownFile}_`)
    }

    if (args.failOnDrift !== undefined) {
      if (Number.isNaN(args.failOnDrift)) { console.error('--fail-on-drift needs a number'); return 2 }
