// dsh-driftwatch — behavioral fingerprinting and drift analysis.
//
// Philosophy: this tool does NOT assert. `dsh-eval-harness` owns pass/fail
// gates and `dsh-replay` owns trajectory debugging. DriftWatch answers a
// different question: "how did this run behave differently from that run?" —
// then lets a human (or a threshold) decide whether the difference matters.
import type { SessionLog, SessionRecord } from './session-log.js'
...
export interface SessionFingerprint {
  ...
  fileOps: { path: string; op: string; turn: number }[]
...
export function fingerprint(log: SessionLog, opts: { maxToolCalls?: number } = {}): SessionFingerprint {
  ...
  const fileOps: { path: string; op: string; turn: number }[] = []
  ...
  for (const rec of records) {
    if (typeof rec.time === 'number') lastTime = Math.max(lastTime, rec.time)
    ...
    switch (rec.type) {
      ...
      case 'reasoning-chunks': {
        ...
        for (const t of texts) if (typeof t === 'string') reasoningChars += t.length
        break
      }
      ...
      case 'tool/call': {
        ...
        if (toolCalls.length < maxToolCalls) {
          ...
          if (path) fileOps.push({ path, op: name, turn })
      ...
      case 'tool/result': {
        ...
        if (isError && callId) {
          ...
          if (idx !== undefined) toolCalls[idx]!.isError = true
        }
        break
      }
      default:
...
export function sequenceDiff(a: string[], b: string[]): SequenceDiff {
  const n = a.length
  const m = b.length
  if (n === 0 && m === 0) return { kept: 0, added: 0, removed: 0, divergence: 0, exact: true }
  ...
  if (n * m > LCS_CELL_BUDGET) {
    ...
    for (const x of b) {
      ...
      if (c > 0) { overlap++; countA.set(x, c - 1) }
    }
...
export interface DriftReport {
  ...
  metrics: MetricDelta[]
  files: { onlyInA: string[]; onlyInB: string[]; common: string[] }
  /** Tool names present in one run only. */
  toolNamesOnlyInA: string[]
  toolNamesOnlyInB: string[]
}

const METRIC_KEYS: (keyof SessionFingerprint)[] = [
...
export function compareFingerprints(a: SessionFingerprint, b: SessionFingerprint): DriftReport {
  ...
  const toolUsage: ToolUsageDelta[] = []
  let l1 = 0
  let totalA = 0
  ...
  const score = Math.round(
    100 * (0.40 * toolSequence.divergence + 0.25 * histogramDrift + 0.20 * metricDrift + 0.15 * fileDrift),
  )
  const verdict: DriftVerdict = score < 10 ? 'stable' : score < 35 ? 'moderate' : 'significant'
