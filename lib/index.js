...
export default Object.assign(function driftwatch(ctx, config = {}) {
    const maxToolCalls = config.maxToolCalls ?? 2000;
    // ── drift_compare ─────────────────────────────────────────────────────
    ctx.tools.register(defineTool({
        name: 'drift_compare',
        description: 'Compare two DSH sessions and report how the agent\'s behavior drifted: tool-call sequence alignment, tool mix, reasoning volume, timing, retries and file targets. ' +
        ...
        async execute(args) {
            const aRef = args.sessionA;
            const bRef = args.sessionB;
